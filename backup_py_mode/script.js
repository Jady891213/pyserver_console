...
`;
    ...
    initDocumentLink(); // 初始化文档链接
    initCopyButtons(); // 初始化复制按钮
    initParamsManager(); // 初始化参数管理功能
    
    // 默认激活运行参数图标
    document.querySelector('#params-btn').click();
...
`;
    ...
    function initDocumentLink() {
        ...
        if (docLink) {
            docLink.addEventListener('click', function() {
                window.open('https://py.deepfos.com/', '_blank');
            });
        }
    }
    ...
    function initParamsManager() {
        ...
        const paramEditModal = document.getElementById('param-edit-modal');
        const paramModalClose = paramEditModal.querySelector('.close');
        const paramCancelBtn = document.getElementById('param-cancel');
        const paramOkBtn = document.getElementById('param-ok');
        
        // 参数表单字段
        const paramCodeInput = document.getElementById('param-code');
        const paramNameInput = document.getElementById('param-name');
        const paramTypeSelect = document.getElementById('param-type');
        const paramDefaultInput = document.getElementById('param-default');
        ...
        // 关闭参数编辑弹窗按钮事件
        paramModalClose.addEventListener('click', function() {
        ...
        // 确定按钮事件 - 添加或更新参数
        paramOkBtn.addEventListener('click', function() {
            // 获取参数数据
            const paramData = {
                code: paramCodeInput.value.trim(),
                name: paramNameInput.value.trim(),
                type: paramTypeSelect.value,
                defaultValue: paramDefaultInput.value.trim(),
                id: paramOkBtn.dataset.paramId
            };
            ...
            if (paramData.id) {
                // 查找并更新参数
                const index = params.findIndex(p => p.id === paramData.id);
                if (index !== -1) {
                    params[index] = { ...paramData };
                }
        } else {
                // 添加新参数
                paramData.id = Date.now().toString(); // 生成唯一ID
            ...
            // 记录操作日志
            logToResultConsole(`${paramData.id ? '更新' : '添加'}参数: ${paramData.code}`);
        });
        
        // 点击弹窗外部关闭
        window.addEventListener('click', function(event) {
            if (event.target === paramEditModal) {
                paramEditModal.style.display = 'none';
            }
        });
            ...
            params.forEach(param => {
                ...
                paramItem.innerHTML = `
                    ...
                    <div class="param-actions">
                        <button class="param-edit-btn" title="编辑参数" data-id="${param.id}">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="param-delete-btn" title="删除参数" data-id="${param.id}">
                ...
                // 添加编辑按钮事件
                paramItem.querySelector('.param-edit-btn').addEventListener('click', function() {
                    const paramId = this.getAttribute('data-id');
                    editParam(paramId);
                });
                
                // 添加删除按钮事件
                paramItem.querySelector('.param-delete-btn').addEventListener('click', function() {
                    const paramId = this.getAttribute('data-id');
                    deleteParam(paramId);
                });
        ...
        // 打开参数编辑弹窗
        function openParamEditModal(paramId = null) {
            // 重置表单
            paramCodeInput.value = '';
            paramNameInput.value = '';
            paramTypeSelect.value = 'string';
            paramDefaultInput.value = '';
            
            // 如果传入参数ID，则填充表单
            if (paramId) {
                const param = params.find(p => p.id === paramId);
                if (param) {
                    paramCodeInput.value = param.code;
                    paramNameInput.value = param.name;
                    paramTypeSelect.value = param.type;
                    paramDefaultInput.value = param.defaultValue;
                    paramOkBtn.dataset.paramId = param.id;
                }
        } else {
                delete paramOkBtn.dataset.paramId;
            }
            
            // 显示弹窗
            paramEditModal.style.display = 'block';
        }
        
        // 编辑参数
        function editParam(paramId) {
            openParamEditModal(paramId);
        }
        ...
        function deleteParam(paramId) {
            ...
            if (confirm('确定要删除此参数吗？')) {
                ...
                if (index !== -1) {
                    // 记录被删除的参数代码
                    const deletedParamCode = params[index].code;
                    ...
                    // 记录操作日志
                    logToResultConsole(`删除参数: ${deletedParamCode}`);
                }
            }
        }
